 ...
 * Same permission matrix as Livraisons Aliment / Produits Hygiène: canCreate for add/save, canUpdate for saved rows, canDelete for delete.
 * Columns: Date, age, designation, fournisseur, qte, prix, montant, N°BL, N°BR, male, femelle.
 * Vide sanitaire row at top (like Produits Hygiène).
 */
...
export default function LivraisonGaz() {
  ...
  const lotReadOnly = hasSavedData; // Lot cannot be modified once data is saved (permissions respected via canUpdate on rows)
  ...
  const handleSave = async () => {
    ...
    const toSend: LivraisonGazRequest[] = rows
    ...
    try {
      ...
      if (toSend.length > 0) {
        await api.livraisonsGaz.createBatch(toSend, pageFarmId ?? undefined);
        toast({
