...
import type { DailyDashboardSummary } from "@/lib/api";

interface DailyMetricsCardProps {
  data: DailyDashboardSummary;
}

export function DailyMetricsCard({ data }: DailyMetricsCardProps) {
  ...
  const maleMetrics = useMemo(
    () => sexMetrics.find((m) => m.sex.toLowerCase().includes("mâle") || m.sex.toLowerCase().includes("male")),
    [sexMetrics]
  );
  
  const femaleMetrics = useMemo(
    () => sexMetrics.find((m) => m.sex.toLowerCase().includes("femelle") || m.sex.toLowerCase().includes("female")),
    [sexMetrics]
  );
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Effectif initial (Effectif Mis en Place) — same data as Reporting Journalier */}
